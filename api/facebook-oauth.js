...
const getConfig = () => {
  ...
  const configId = String(process.env.FACEBOOK_CONFIG_ID || "").trim();
  ...
  return { appId, appSecret, redirectUri, configId, scope };
};
...
const handleStart = async (res) => {
  applyNoStore(res);
  const { appId, redirectUri, configId, scope } = getConfig();
  if (!appId || !redirectUri || !configId) return res.status(400).json({ error: "Missing FACEBOOK_APP_ID/META_APP_ID, FACEBOOK_REDIRECT_URI, or FACEBOOK_CONFIG_ID" });
...
const handleExchange = async (req, res) => {
  ...
  const incomingAccessToken = String(req.body?.access_token || req.query?.access_token || "").trim();
  if (!code && !incomingAccessToken) return res.status(400).json({ error: "code or access_token is required" });
  const { appId, appSecret, redirectUri } = getConfig();
  if (!incomingAccessToken && (!appId || !appSecret || !redirectUri)) return res.status(400).json({ error: "Missing FACEBOOK_APP_ID/META_APP_ID, FACEBOOK_APP_SECRET/META_APP_SECRET, or FACEBOOK_REDIRECT_URI" });

  const supabase = buildSupabase();
  try {
    const tokenPayload = incomingAccessToken
      ? { access_token: incomingAccessToken, expires_in: Number(req.body?.expires_in || req.query?.expires_in || 0) }
      : await fetchJson(`${GRAPH_BASE}/oauth/access_token?client_id=${encodeURIComponent(appId)}&client_secret=${encodeURIComponent(appSecret)}&redirect_uri=${encodeURIComponent(redirectUri)}&code=${encodeURIComponent(code)}`);
