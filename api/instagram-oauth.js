...
const handleCallbackCompat = async (req, res) => {
  ...
  if (mode === "json") {
    if (error) return res.status(200).json({ success: false, error, errorReason, errorDescription });
...
export default async function handler(req, res) {
  const op = getOp(req);
  try {
    if (req.method === "GET" && !op && (req.query?.code || req.query?.error || req.query?.mode)) return await handleCallbackCompat(req, res);
