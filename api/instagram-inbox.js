...
const fetchConversationsFromMeta = async ({ accessToken, igUserId }) => {
  ...
  for (const path of paths) {
    try {
      ...
      attempts.push({ endpoint: path, error: String(error?.message || error) });
    }
  }
  const details = attempts.map((a) => `${a.endpoint} -> ${a.error}`).join(" | ");
  throw new Error(`Instagram inbox endpoints failed. ${details}`);
};
...
const handleSync = async (res) => {
  ...
  try {
    ...
    return res.status(200).json({
      ...
      emptyReason: rows.length ? "" : "Meta returned empty data for conversations",
    });
  } catch (error) {
