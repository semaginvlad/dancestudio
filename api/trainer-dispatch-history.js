...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    const { data, error } = await supabase
      ...
      .limit(limit);
    if (error) return res.status(500).json({ error: "Failed to load trainer dispatch history", details: String(error.message || error) });
  ...
  if (req.method === "POST") {
    ...
    const { data, error } = await supabase
      ...
      .select("*")
      .single();
    if (error) return res.status(500).json({ error: "Failed to save trainer dispatch history", details: String(error.message || error) });
