...
const pctDelta = (curr, prev) => (prev > 0 ? Number((((curr - prev) / prev) * 100).toFixed(1)) : (curr > 0 ? 100 : 0));
...
const Ring = ({ value, label }) => {
  ...
  const cl = Math.max(0, Math.min(140, value));
  return <div style={{ display: "grid", justifyItems: "center", gap: 4 }}>
    <svg width="78" height="78" viewBox="0 0 78 78">
      <circle cx="39" cy="39" r={r} fill="none" stroke={theme.border} strokeWidth="8" />
      <circle cx="39" cy="39" r={r} fill="none" stroke={ringColor(value)} strokeWidth="8" strokeLinecap="round" transform="rotate(-90 39 39)" strokeDasharray={`${(cl / 100) * c} ${c}`} />
      <text x="39" y="43" textAnchor="middle" style={{ fill: theme.textMain, fontWeight: 800, fontSize: 13 }}>{value}%</text>
    </svg>
    <div style={{ fontSize: 11, color: theme.textMuted }}>{label}</div>
  </div>;
};

const TrendLine = ({ rows = [], title, color, delta }) => {
  if (rows.length < 2) return <div style={{ ...cardSt, border: `1px solid ${theme.border}` }}><b>{title}</b><div style={{ color: theme.textLight, marginTop: 8 }}>немає достатньо даних</div></div>;
  const w = 420; const h = 180; const p = 26;
  ...
  return <div style={{ ...cardSt, border: `1px solid ${theme.border}` }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      <b>{title}</b>
      <div style={{ color: delta > 0 ? theme.success : delta < 0 ? theme.danger : theme.textMuted, fontWeight: 700 }}>{delta > 0 ? "▲" : delta < 0 ? "▼" : "•"} {delta}%</div>
    </div>
    <svg width="100%" height={h} viewBox={`0 0 ${w} ${h}`}>
      <line x1={p} y1={h - p} x2={w - p} y2={h - p} stroke={theme.border} />
      <line x1={p} y1={p} x2={p} y2={h - p} stroke={theme.border} />
      <polyline fill="none" stroke={color} strokeWidth="3" points={points} />
      <text x={p} y={h - 6} fontSize="10" fill={theme.textLight}>{fmtShort(first.day)}</text>
      <text x={w / 2} y={h - 6} fontSize="10" fill={theme.textLight} textAnchor="middle">{fmtShort(mid.day)}</text>
      <text x={w - p} y={h - 6} fontSize="10" fill={theme.textLight} textAnchor="end">{fmtShort(last.day)}</text>
      <text x={p - 4} y={p + 4} fontSize="10" fill={theme.textLight} textAnchor="end">max {max}</text>
      <text x={p - 4} y={h - p} fontSize="10" fill={theme.textLight} textAnchor="end">min {min}</text>
      <text x={w - p} y={p + 4} fontSize="10" fill={theme.textMain} textAnchor="end">last {last.value}</text>
    </svg>
  </div>;
};

const RankList = ({ title, rows = [], unit = "" }) => <div style={{ ...cardSt, border: `1px solid ${theme.border}` }}><b>{title}</b><div style={{ display: "grid", gap: 7, marginTop: 8 }}>{rows.map((r, i) => <div key={`${title}_${r.id || r.name}`} style={{ display: "flex", justifyContent: "space-between", fontSize: 12 }}><span>{i + 1}. {r.name}</span><b>{(r.value || 0).toLocaleString()}{unit}</b></div>)}</div></div>;

export default function DashboardTab({ students = [], groups = [], directionsList = [], subs = [], attn = [], waitlist = [] }) {
  const now = new Date();
  ...
  const calc = (range) => {
    const pSubs = subs.filter((s) => inRange(String(s.activationDate || s.startDate || "").slice(0, 10), range.start, range.end));
    ...
    const byDayRev = new Map(); pSubs.forEach((s) => { const d = String(s.activationDate || s.startDate || "").slice(0, 10); byDayRev.set(d, (byDayRev.get(d) || 0) + Number(s.amount || 0)); });
    ...
    return {
      ...
      byDirRevenue: directionsList.map((d) => {
        const gids = groups.filter((g) => String(g.directionId) === String(d.id)).map((g) => String(g.id));
        return { id: d.id, name: d.name, value: pSubs.filter((s) => gids.includes(String(s.groupId))).reduce((sum, s) => sum + Number(s.amount || 0), 0) };
      }),
    };
  };
  ...
  const health = curr.byGroupAttendance.map((g) => {
  ...
  const works = [];
  curr.byDirRevenue.forEach((d) => {
    const prevDir = prevData.byDirRevenue.find((x) => String(x.id) === String(d.id))?.value || 0;
    if (d.value > prevDir && d.value > 0) works.push(`Напрямок ${d.name} виріс по виручці (${pctDelta(d.value, prevDir)}%).`);
  });
  curr.byGroupAttendance.forEach((g) => { if (g.held >= 2 && (g.value / g.held) >= 6) works.push(`Група ${g.name}: висока сер. відвідуваність (${(g.value / g.held).toFixed(1)}).`); });

  const weak = [];
  curr.byGroupAttendance.forEach((g) => {
    const prevG = prevData.byGroupAttendance.find((x) => String(x.id) === String(g.id))?.value || 0;
    if (prevG > 0 && g.value < prevG) weak.push(`Група ${g.name}: відвідуваність просіла (${pctDelta(g.value, prevG)}%).`);
    if (g.held >= 2 && (g.value / g.held) < 4) weak.push(`Група ${g.name}: низька сер. відвідуваність (${(g.value / g.held).toFixed(1)}).`);
  });

  const targetCards = [
    { key: "revenue", label: "Виручка", actual: curr.revenue, target: targets.revenueTarget, unit: "₴" },
    { key: "attendance", label: "Відвідуваність", actual: curr.attendance, target: targets.attendanceTarget, unit: "" },
    { key: "active", label: "Активні учениці", actual: curr.activeStudents, target: targets.activeStudentsTarget, unit: "" },
    { key: "recruit", label: "Набір / оплати", actual: curr.payments, target: targets.recruitmentTarget, unit: "" },
  ];
  ...
  return <div style={{ display: "grid", gap: 14 }}>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 12 }}>{targetCards.map((c) => { const pc = Math.max(0, Math.min(140, pctDelta(c.actual, 0) && (c.target > 0 ? Math.round((c.actual / c.target) * 100) : 0))); return <div key={c.key} style={{ ...cardSt, border: `1px solid ${theme.border}`, display: "grid", gridTemplateColumns: "auto 1fr", gap: 10 }}><Ring value={pc} label={rank(pc)} /><div><div style={{ fontSize: 12, color: theme.textMuted }}>{c.label}</div><div style={{ fontSize: 24, fontWeight: 800 }}>{c.actual.toLocaleString()}{c.unit}</div><div style={{ fontSize: 12, color: theme.textLight }}>План: {Number(c.target || 0).toLocaleString()}{c.unit}</div><input type="number" value={targets[`${c.key === "active" ? "activeStudents" : c.key === "recruit" ? "recruitment" : c.key}Target`] || 0} onChange={(e) => setTargets((p) => ({ ...p, [`${c.key === "active" ? "activeStudents" : c.key === "recruit" ? "recruitment" : c.key}Target`]: Number(e.target.value || 0) }))} style={{ marginTop: 6, width: 150 }} /></div></div>; })}</div>

    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
      <TrendLine title="Тренд виручки" rows={curr.revenueTrend} color={theme.success} delta={pctDelta(curr.revenue, prevData.revenue)} />
      <TrendLine title="Тренд відвідуваності" rows={curr.attendanceTrend} color={theme.primary} delta={pctDelta(curr.attendance, prevData.attendance)} />
    </div>

    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 12 }}>
      <RankList title="Топ 5 груп за виручкою" rows={curr.byGroupRevenue.sort((a,b)=>b.value-a.value).slice(0,5)} unit="₴" />
      <RankList title="Топ 5 груп за відвідуваністю" rows={curr.byGroupAttendance.sort((a,b)=>b.value-a.value).slice(0,5)} />
      <RankList title="Топ 5 напрямків за виручкою" rows={curr.byDirRevenue.sort((a,b)=>b.value-a.value).slice(0,5)} unit="₴" />
    </div>
