    ...
    return (
      <div key={w.id} style={{
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end" }}>
          <button style={{ ...btnS, padding: "10px 12px", fontSize: 13 }} onClick={async () => { if (!db.updateWaitlist) return; const next = await db.updateWaitlist(w.id, { status: "contacted" }); setWaitlist(p => p.map(x => x.id === w.id ? next : x)); }}>Написали</button>
          <button style={{ ...btnS, padding: "10px 12px", fontSize: 13 }} onClick={async () => { if (!w.studentId) { alert("Для joined потрібна існуюча учениця"); return; } try { const link = await db.addStudentGroup(w.studentId, w.groupId); setStudentGrps(p => p.some(sg => String(sg.studentId) === String(w.studentId) && String(sg.groupId) === String(w.groupId)) ? p : [...p, link || { id: uid(), studentId: w.studentId, groupId: w.groupId }]); if (db.updateWaitlist) { const next = await db.updateWaitlist(w.id, { status: "joined" }); setWaitlist(p => p.map(x => x.id === w.id ? next : x)); } } catch (e) { console.warn(e); } }}>Додати в групу</button>
          <button style={{ ...btnS, padding: "10px 12px", fontSize: 13, color: theme.danger, background: theme.input }} onClick={async () => { if (!db.updateWaitlist) return; const next = await db.updateWaitlist(w.id, { status: "removed" }); setWaitlist(p => p.map(x => x.id === w.id ? next : x)); }}>Прибрати</button>
        </div>
      </div>
    );
  };
