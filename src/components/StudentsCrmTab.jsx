...
export default function StudentsCrmTab({
  ...
  const activeWaitlistStatuses = new Set(["waiting", "contacted", "offered"]);
  const completedWaitlistStatuses = new Set(["joined", "declined", "removed"]);
  const activeWaitlist = waitlist.filter((w) => activeWaitlistStatuses.has(String(w.status || "waiting")));
  const completedWaitlist = waitlist.filter((w) => completedWaitlistStatuses.has(String(w.status || "waiting")));
  const visibleWaitlistByMode = waitlistMode === "completed" ? completedWaitlist : activeWaitlist;
  ...
  const waitlistStatusOptions = [
    ...
    { status: "contacted", label: "Написали", color: theme.primary || "#2563eb", background: "rgba(59, 130, 246, 0.14)" },
  ...
  const getWaitlistStatusMeta = (status) => waitlistStatusOptions.find((item) => item.status === status) || waitlistStatusOptions[0];
  ...
  const removeWaitlistEntry = async (w) => {
    if (!window.confirm("Прибрати заявку з активного резерву?")) return;
    try {
  ...
  const renderWaitlistStatusSelector = (w, status, className = "") => {
    const current = getWaitlistStatusMeta(status);
    const isOpen = String(openWaitlistStatusId || "") === String(w.id);

    return (
      <div className={`student-waitlist-status-wrap ${className}`} style={{ position: "relative", display: "inline-flex", justifyContent: "flex-end" }}>
        <button
          type="button"
          className="student-waitlist-status"
          onClick={() => setOpenWaitlistStatusId((prev) => (String(prev || "") === String(w.id) ? null : w.id))}
          style={{
            display: "inline-flex",
            alignItems: "center",
            gap: 5,
        ...
        {isOpen ? (
          <div
            className="student-waitlist-status-menu"
            style={{
              position: "absolute",
              top: "calc(100% + 6px)",
              right: 0,
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    ...
                    updateWaitlistStatus(w, option.status);
                  }}
                  style={{
                    width: "100%",
                    ...
                    color: active ? option.color : theme.textMain,
                    padding: "8px 9px",
                    textAlign: "left",
                    fontSize: 12.5,
  ...
  const renderWaitlistModeSwitch = () => (
    <div className="student-waitlist-mode-switch" style={{ display: "inline-flex", gap: 6, padding: 4, borderRadius: 999, background: theme.card, border: `1px solid ${theme.border}`, marginBottom: 12, maxWidth: "100%", overflowX: "auto" }}>
      {[
        { mode: "active", label: "Активні", count: activeWaitlist.length },
        { mode: "completed", label: "Завершені", count: completedWaitlist.length },
      ].map((item) => {
        const selected = waitlistMode === item.mode;
        return (
          <button
            key={item.mode}
            type="button"
            onClick={() => setWaitlistMode(item.mode)}
            style={{
              border: "none",
              borderRadius: 999,
              background: selected ? (theme.warning || "#f59e0b") : "transparent",
      ...
      <div key={w.id} className="student-waitlist-card" style={{
        ...
        <div className="student-waitlist-main" style={{ display: "flex", gap: 9, alignItems: "flex-start", minWidth: 0 }}>
          ...
          <div className="student-waitlist-mobile-meta" style={{ display: "none" }}>
            ...
            {renderWaitlistStatusSelector(w, status)}
          </div>
        </div>
        <div className="student-waitlist-info" style={{ minWidth: 0 }}>
          ...
          {renderWaitlistStatusSelector(w, status, "student-waitlist-desktop-status")}
        </div>
    ...
    <div className="students-crm-root" style={{ display: "grid", gap: 18, minWidth: 0, overflowX: "hidden" }}>
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .student-waitlist-status-menu { right: 0 !important; max-width: calc(100vw - 34px) !important; }
          .student-waitlist-status-menu button { min-height: 34px !important; padding: 7px 8px !important; font-size: 12px !important; flex: none !important; }
          .student-waitlist-mode-switch { width: 100% !important; justify-content: stretch !important; margin-bottom: 8px !important; padding: 3px !important; gap: 4px !important; }
          .student-waitlist-mode-switch button { flex: 1 1 0 !important; min-height: 34px !important; padding: 6px 8px !important; font-size: 11.5px !important; }
      ...
      <div style={{ display: "grid", gap: 18 }}>
        ...
        {shouldShowReserveSection && (
          <section className="students-section" style={{ background: theme.input, border: `1px solid ${theme.border}`, borderRadius: 26, padding: 18 }}>
            {renderSectionHeader("Резерв / потенційні", waitlistMode === "completed" ? completedWaitlist.length : activeWaitlist.length, waitlistMode === "completed" ? "Завершені заявки: joined, declined, removed" : "Активні заявки: waiting, contacted, offered", theme.warning || "#f59e0b")}
            {renderWaitlistModeSwitch()}
            {waitlistMode === "active" && reserveGroupHints.length > 0 && (
            ...
            {visibleWaitlistByMode.length > 0 ? (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {visibleWaitlistByMode.map((w, i) => renderWaitlistCard(w, i))}
              </div>
            ) : (selectedSummaryFilter === "reserve" ? filterEmptyState : (
              <div style={{ background: theme.card, border: `1px dashed ${theme.border}`, borderRadius: 18, padding: 22, color: theme.textMuted, fontWeight: 800, textAlign: "center" }}>{waitlistMode === "completed" ? "Завершених заявок резерву поки немає." : "Активного резерву поки немає."}</div>
            ))}
          </section>
        )}
