...
export default function StudentsCrmTab({
  ...
  const buildStudentFromWaitlist = (w) => {
    ...
    const looksTelegram = contact.includes("@") || contact.startsWith("@");
    const looksPhone = digitsOnly.length >= 7 && /^[+\d\s().-]+$/.test(contact);
    const notes = [w.note, !looksTelegram && !looksPhone && contact ? `Контакт з резерву: ${contact}` : ""].filter(Boolean).join("\n");

    return {
      name: String(w.name || "Новий контакт").trim() || "Новий контакт",
      phone: looksPhone ? contact : "",
      telegram: looksTelegram ? contact : "",
      notes,
    };
  };

  const handleMarkWaitlistContacted = async (w) => {
    try {
      const next = await db.updateWaitlist(w.id, { status: "contacted" });
      setWaitlist((prev) => prev.map((x) => x.id === w.id ? next : x));
    } catch (e) {
      console.error("Failed to mark waitlist contacted:", e);
      alert(`Не вдалося оновити резерв: ${e?.message || e}`);
    }
  };

  const handleMoveWaitlistToJoined = async (w) => {
    try {
      ...
      setStudentGrps((prev) => prev.some((sg) => String(sg.studentId) === String(studentId) && String(sg.groupId) === String(w.groupId)) ? prev : [...prev, link || { id: uid(), studentId, groupId: w.groupId }]);

      if (createdStudent) {
        setStudents((prev) => prev.some((st) => String(st.id) === String(createdStudent.id)) ? prev : [createdStudent, ...prev]);
      }

      const next = await db.updateWaitlist(w.id, { studentId, status: "joined" });
      setWaitlist((prev) => prev.map((x) => x.id === w.id ? next : x));
    } catch (e) {
      console.error("Failed to move waitlist entry to joined:", e);
      alert(`Не вдалося перенести з резерву в групу: ${e?.message || e}`);
    }
  };

  const handleRemoveWaitlist = async (w) => {
    try {
              ...
              return (
                <div key={w.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center", background: theme.card, padding: "20px", borderRadius: 20}}>
                  ...
                  <div style={{display:"flex", gap:8, flexWrap:"wrap"}}>
                    <button style={{...btnS,padding:"10px 12px",fontSize:13}} onClick={() => handleMarkWaitlistContacted(w)}>Позначити contacted</button>
                    <button style={{...btnS,padding:"10px 12px",fontSize:13}} onClick={() => handleMoveWaitlistToJoined(w)}>Move to joined</button>
                    <button style={{...btnS,padding:"10px 12px",fontSize:13,color:theme.danger, background: theme.input}} onClick={() => handleRemoveWaitlist(w)}>Remove</button>
                  </div>
                </div>
              )
            })}
