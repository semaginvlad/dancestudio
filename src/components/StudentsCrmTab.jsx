        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "flex-end", alignItems: "flex-start", minWidth: 0 }}>
          ...
          {!isHistory && (
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "flex-end" }}>
              {trialStatusActions.filter((action) => action.status !== status).map((action) => (
                <button key={action.status} style={{ ...btnS, padding: "8px 10px", fontSize: 12 }} onClick={() => updateTrialBookingStatus(booking, action.status)}>
                  {action.label}
                </button>
              ))}
            </div>
