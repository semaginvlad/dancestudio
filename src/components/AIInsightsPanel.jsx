...
export default function AIInsightsPanel({ isAdmin = false, context = {} }) {
  ...
  const insights = useMemo(() => (generated ? buildAIInsights(context) : []), [context, generated]);
    ...
    <section style={{ ...cardSt, border: `1px solid ${theme.border}`, display: "grid", gap: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
        <div>
          ...
          <div style={{ color: theme.textMuted, fontSize: 13 }}>Read-only підказки на основі CRM-метрик. Без LLM, без запису в базу, без автоматичних повідомлень.</div>
        </div>
        <button type="button" style={{ ...btnP, whiteSpace: "nowrap" }} onClick={() => setGenerated(true)}>
          Згенерувати підказки
        </button>
      </div>

      {!generated ? (
        <div style={{ padding: 16, borderRadius: 16, background: theme.input, color: theme.textMuted, fontSize: 13 }}>
          Натисни кнопку, щоб вручну сформувати 3–5 безпечних підказок для адміністратора. Панель не рендериться для тренерів.
        </div>
      ) : (
