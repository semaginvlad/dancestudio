...
const MessageBubble = React.memo(function MessageBubble({ message, chatId, onReact, onOpenPicker, onQuoteTap, highlighted }) {
  ...
  const handlePointerUp = useCallback((event) => {
    clearLongPress();
    if (suppressNextTapRef.current || pointerRef.current.longPressed || isInteractiveReactionTarget(event.target)) {
          ...
          <div className="messages-reaction-chips" aria-label="Реакції Telegram">
            {reactions.map((reaction) => (
              <button
                ...
                onPointerDown={(event) => event.stopPropagation()}
                onClick={(event) => { event.stopPropagation(); onReact(message, reaction.chosen ? "" : reaction.emoji); }}
...
export default function MessagesTab({
  ...
  const handleSetReaction = useCallback(async (message, emoji) => {
    ...
    setReactionError("");
    triggerHaptic(emoji ? 15 : 8);
    setReactionPicker((prev) => prev ? { ...prev, selectedEmoji: emoji || "remove" } : null);
    window.setTimeout(() => setReactionPicker(null), 130);
  ...
  const copyMessageText = useCallback(async (text) => {
    ...
    try {
      ...
      setCopyFeedback("Скопійовано");
      window.setTimeout(() => setCopyFeedback(""), 1800);
      setReactionPicker(null);
    } catch (error) {
      setReactionError("Не вдалося скопіювати текст");
      window.setTimeout(() => setReactionError(""), 3200);
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {!!copyFeedback && <div className="messages-reaction-error messages-copy-feedback" role="status">{copyFeedback}</div>}
      ...
      {reactionPicker && activeDialog && (
        <div className="messages-reaction-picker-layer" role="presentation">
          ...
          <div className="messages-reaction-picker" role="dialog" aria-label="Вибір реакції" style={{ left: `min(max(${reactionPicker.x - 120}px, 10px), calc(100vw - 260px))`, top: `min(max(${reactionPicker.y - 68}px, 10px), calc(100dvh - 116px))` }}>
            <div className="messages-reaction-picker-emojis">
              {SAFE_TELEGRAM_REACTIONS.map((emoji) => {
                ...
                return (
                  <button
                    key={emoji}
                    type="button"
                    className={`messages-reaction-picker-option ${chosen ? "is-chosen" : ""} ${reactionPicker.selectedEmoji === emoji ? "is-selected" : ""}`}
                    aria-label={`${chosen ? "Видалити" : "Поставити"} реакцію ${emoji}`}
                    onClick={() => handleSetReaction(msg, chosen ? "" : emoji)}
                  >
                    {emoji}
                  </button>
                );
      ...
      <style>{`
        ...
        .messages-message-row .messages-bubble { max-width: 100% !important; }
        .messages-reaction-gesture { touch-action: pan-y; user-select: none; -webkit-user-select: none; -webkit-touch-callout: none; transform-origin: center center; transition: transform 120ms ease, box-shadow 120ms ease, filter 120ms ease; will-change: transform; }
        .messages-reaction-gesture.is-pressed { transform: scale(.988); filter: brightness(1.04); box-shadow: 0 8px 18px rgba(0,0,0,.16); }
        .messages-reaction-chips { display: flex; flex-wrap: wrap; gap: 4px; margin-top: 4px; max-width: 100%; }
        .messages-reaction-chip { min-height: 32px; transition: transform 110ms ease, background 110ms ease; min-width: 34px; display: inline-flex; align-items: center; justify-content: center; gap: 4px; border: 0; border-radius: 999px; padding: 3px 8px; background: rgba(142,142,147,.14); color: ${theme.textMuted}; font-size: 12px; font-weight: 800; cursor: pointer; }
        .messages-reaction-chip:active { transform: scale(.94); }
        ...
        .messages-reaction-picker { position: absolute; display: grid; gap: 5px; padding: 7px; border: 1px solid ${theme.border}; border-radius: 18px; background: ${theme.card}; box-shadow: 0 18px 38px rgba(0,0,0,.25); pointer-events: auto; transform-origin: 50% 100%; animation: messagesReactionPickerIn 140ms ease-out both; padding-bottom: max(7px, env(safe-area-inset-bottom)); }
        .messages-reaction-picker-emojis { display: flex; gap: 4px; }
        .messages-reaction-picker-option { width: 36px; height: 36px; border: 0; border-radius: 999px; background: transparent; font-size: 20px; cursor: pointer; transition: transform 110ms ease, background 110ms ease; }
        .messages-reaction-picker-option:active, .messages-reaction-picker-option.is-selected { transform: scale(.86); }
        .messages-reaction-picker-option.is-chosen { background: ${theme.primary}20; }
        .messages-reaction-copy { min-height: 32px; border: 0; border-radius: 999px; background: rgba(142,142,147,.14); color: ${theme.textMain}; font-size: 12px; font-weight: 800; cursor: pointer; }
        @keyframes messagesReactionPickerIn { from { opacity: 0; transform: scale(.94); } to { opacity: 1; transform: scale(1); } }
        ...
        .messages-copy-feedback { background: ${theme.success}; }
        @media (prefers-reduced-motion: reduce) {
          .messages-reaction-gesture, .messages-reaction-chip, .messages-reaction-picker-option { transition: none !important; }
          .messages-reaction-picker { animation: none !important; }
        }
