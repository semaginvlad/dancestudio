  ...
  const notificationRecipients = useMemo(() => {
    const mapped = (trainers || []).map((t) => {
      ...
      return {
        ...
        subtitle: contact || authId || fallbackId || "—",
            ...
            {showMessageTemplate && (
              <>
                <textarea
                  ...
                  rows={4}
                  placeholder={"Наприклад: Не забудь перевірити пробні та оплати по групі {group_name}"}
                  style={{ ...fieldStyle, resize: "vertical" }}
                />
                <div style={{ fontSize: 12, color: theme.textMuted }}>Можна залишити порожнім. Основний текст система сформує автоматично з актуальних даних групи.</div>
              </>
            )}
