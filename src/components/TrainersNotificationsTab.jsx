  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        try {
          const rRes = await fetch("/api/trainer-notification-readiness");
      ...
      <div style={{ background: theme.card, border: `1px solid ${theme.border}`, borderRadius: 16, padding: 12, display: "grid", gap: 10 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
          <div>
            ...
            <div style={{ fontSize: 12, color: readiness.adminConfigured ? theme.success : theme.warning }}>
              Адмін-сповіщення: {readiness.adminConfigured ? "налаштовано" : "не налаштовано"}
            </div>
            <div style={{ fontSize: 11, color: readiness.ready ? theme.textMuted : theme.warning }}>
              Storage: {readiness.ready ? "готовий" : "не готовий (застосуйте SQL migrations)"}
            </div>
          </div>
          <div style={{ display: "flex", gap: 6 }}>
            <button type="button" onClick={regenerateFromTemplate} style={{ border: `1px solid ${theme.border}`, borderRadius: 10, background: theme.input, color: theme.textMain, padding: "6px 8px", cursor: "pointer", fontWeight: 700, fontSize: 12 }}>Шаблон</button>
            ...
            <button type="button" onClick={copyToClipboard} style={{ border: "none", borderRadius: 10, background: theme.primary, color: "#fff", padding: "6px 8px", cursor: "pointer", fontWeight: 700, fontSize: 12 }}>Копія</button>
            ...
            <button type="button" disabled={sendingNow} onClick={() => sendNow({ dryRun: false })} style={{ border: "none", borderRadius: 10, background: theme.success, color: "#fff", padding: "6px 8px", cursor: "pointer", fontWeight: 700, fontSize: 12 }}>{sendingNow ? "..." : "Надіслати"}</button>
          </div>
        </div>
        ...
        <div style={{ display: "grid", gap: 6 }}>
          <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
            ...
            />
            <button type="button" onClick={() => upsertGroupState(activeGroupId, { sendTimeOverride: sendOverrideDraft || null })} style={{ border: `1px solid ${theme.border}`, borderRadius: 8, background: theme.card, color: theme.textMain, padding: "4px 6px", cursor: "pointer", fontSize: 12 }}>Apply</button>
            <button type="button" onClick={() => { setSendOverrideDraft(""); upsertGroupState(activeGroupId, { sendTimeOverride: null }); }} style={{ border: `1px solid ${theme.border}`, borderRadius: 8, background: theme.card, color: theme.textMain, padding: "4px 6px", cursor: "pointer", fontSize: 12 }}>Reset</button>
          </div>
        </div>
