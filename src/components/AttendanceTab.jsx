...
export default function AttendanceTab({
  ...
  const handleCreateGuestAttendance = async () => {
    if (!gid) return;
    const guestLabel = (guestNameInput || "").trim() || buildGuestAutoLabel();
    try {
      await db.insertAttendance({
        ...
        guestType: guestEntryType,
        groupId: gid,
        quantity: 1,
        entryType: guestEntryType,
      });
      setGuestNameInput("");
      await reloadFromDb();
    } catch (err) {
  ...
  const handleConvertGuestToStudent = async (guestRow) => {
    ...
    const defaultName = guestRow.guestName;
  ...
  const guestRows = useMemo(() => {
    ...
    rows.forEach((r) => {
      ...
      const existing = grouped.get(key) || { id: `guest:${key}`, isGuest: true, guestName: baseName, attendanceIds: [] };
      existing.attendanceIds.push(r.id);
      grouped.set(key, existing);
    });
    return Array.from(grouped.values());
  }, [attn, gid, visibleDays]);
                  ...
                  <tr key={student.id}>
                    <td style={styles.rowHead}>
                      <div style={styles.studentNameRow}>
                        <div style={styles.studentName}>{`${rowIndex + 1}. ${student.guestName}`}</div>
                    ...
                    {visibleDays.map((dateStr) => {
                      ...
                      return (
                        <td key={dateStr} style={styles.cell(isCancelledDate(dateStr), dateStr.slice(0, 7) !== centerMonth, dateStr.slice(0, 7) === centerMonth)}>
                          <button type="button" disabled style={styles.cellBtn(cellView.bg, true, false)}>{cellView.mark}</button>
                        </td>
                      );
                    })}
                  </tr>
