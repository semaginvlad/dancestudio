...
import { fetchStudioRooms, createStudioRoom, updateStudioRoom } from "../db";
...
export default function ScheduleTab({
  ...
  const getDraftTitle = (value, eventType) => {
  ...
  const saveBooking = async () => {
    ...
    const normalizedTitle = getDraftTitle(draft.title, draft.eventType);
    const missing = {
      date: !draft.date,
    ...
    if (missing.date || missing.title || missing.start || missing.end) {
      console.warn("[ScheduleTab] Invalid booking draft", { missing, draft });
  ...
  const saveQuickCreate = async () => {
    const normalizedTitle = getDraftTitle(quickCreate?.title, quickCreate?.eventType);
  ...
  const saveRenameRoom = async (room) => {
    ...
    try {
      await updateStudioRoom(room.id, { name: nextName });
        ...
        <div style={{ ...cardSt, border: `1px solid ${theme.border}` }}>
          ...
          <div style={{ marginTop: 8, display: "flex", gap: 6, flexWrap: "wrap" }}>
            {studioRooms.map((room) => {
              const isDefaultRoom = normalizeRoomName(room.name) === DEFAULT_ROOM;
              ...
              return (
                <span key={room.id} style={{ padding: "6px 10px", border: `1px solid ${theme.border}`, borderRadius: 999, display: "inline-flex", gap: 8, alignItems: "center" }}>
                  {isRenaming ? (
                    ...
                    <>
                      {room.name}
                      {!isDefaultRoom ? <button type="button" style={{ ...btnS, padding: "0 6px", lineHeight: "16px" }} onClick={() => startRenameRoom(room)}>✎</button> : null}
                      {!isDefaultRoom ? <button type="button" style={{ ...btnS, padding: "0 6px", lineHeight: "16px" }} onClick={() => removeCustomRoom(room)}>×</button> : null}
                    </>
                  )}
                </span>
              );
