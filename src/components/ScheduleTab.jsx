...
export default function ScheduleTab({
  ...
  const buildEventsByDayForDates = useMemo(() => (dates = []) => {
    ...
    safeGroups.forEach((g) =>
      (Array.isArray(g.schedule) ? g.schedule : []).forEach((row, idx) => {
        ...
        const regularTrainerId = row.trainerId ?? row.trainer_id ?? g.trainer_id ?? null;
        const regularTitle = String(row.title || row.name || row.label || "").trim() || g.name || g.id;
        slots.push({
          id: `${g.id}_${idx}`,
          groupId: g.id,
          slotIndex: idx,
          ...
          direction: getDirectionDisplayName(row.direction || dirMap.get(String(g.directionId || "")) || "—"),
          trainerId: regularTrainerId,
          trainer: trainerMap.get(String(regularTrainerId || "")) || "—",
  ...
  const openGroupSlotEditor = (e) => {
    ...
    const row = Array.isArray(g?.schedule) ? g.schedule[e.slotIndex] || {} : {};
    const wd = parseWeekday(row.weekday ?? row.dayOfWeek ?? row.day ?? row.dow ?? row.weekDay) ?? e.weekday;
    const st = toMin(row.startTime || row.start || row.time || e.startTime || "");
    const en = toMin(row.endTime || row.end || e.endTime || "") ?? (st != null ? st + 60 : null);
    const regularTrainerId = row.trainerId ?? row.trainer_id ?? g?.trainer_id ?? e.trainerId ?? "";
    const regularTitle = String(row.title || row.name || row.label || "").trim();
    setGroupSlotEdit({
      groupId: e.groupId,
      ...
      roomName: getRoomLabel(row) || getRoomLabel(g) || e.roomName || primaryRoomName,
      trainerId: regularTrainerId || "",
      title: regularTitle,
      note: row.note || "",
      direction: e.direction || getDirectionDisplayName(dirMap.get(String(g?.directionId || "")) || "—"),
      trainer: trainerMap.get(String(regularTrainerId || "")) || e.trainer || "—",
      error: "",
    });
  };
  ...
  const saveGroupSlotEdit = async () => {
    if (!groupSlotEdit || !onUpdateGroupSchedule || groupSlotEdit.error || !isAdmin) return;
    ...
    const nextSchedule = prev.map((row, idx) => {
      ...
      if ("time" in next) next.time = `${groupSlotEdit.startTime}-${groupSlotEdit.endTime || ""}`.replace(/-$/, "");
      if ("endTime" in next) next.endTime = groupSlotEdit.endTime;
      else if ("end" in next) next.end = groupSlotEdit.endTime;
      else next.endTime = groupSlotEdit.endTime;
      next.roomName = normalizeRoomName(groupSlotEdit.roomName || primaryRoomName) || primaryRoomName;
      next.trainerId = groupSlotEdit.trainerId || null;
      if ("trainer_id" in next) next.trainer_id = groupSlotEdit.trainerId || null;
      next.title = String(groupSlotEdit.title || "").trim() || null;
      next.note = String(groupSlotEdit.note || "").trim() || null;
      return next;
    });
    try {
      ...
      {isAdmin && groupSlotEdit && (
        <div style={{ position: "fixed", inset: 0, zIndex: 5050, background: editorOverlay, display: "grid", placeItems: isMobile ? "end center" : "center", padding: isMobile ? "0 8px" : 12 }}>
          <div style={{ ...cardSt, border: `1px solid ${theme.border}`, width: isMobile ? "calc(100vw - 16px)" : "min(540px,96vw)", maxHeight: isMobile ? "74vh" : "86vh", overflowY: "auto", borderRadius: isMobile ? "16px 16px 0 0" : 18, background: editorSurface, backdropFilter: "blur(22px) saturate(1.2)", WebkitBackdropFilter: "blur(22px) saturate(1.2)" }}>
            ...
            <div style={{ display: "grid", gap: 8 }}>
              <div style={editorSectionLabelSt}>Назва і повтор</div>
              <input style={editorInputSt} placeholder={groupSlotEdit.groupName || "Назва заняття"} value={groupSlotEdit.title || ""} onChange={(e) => setGroupSlotEdit((p) => ({ ...p, title: e.target.value }))} />
              <select style={editorInputSt} value={groupSlotEdit.weekday} onChange={(e) => setGroupSlotEdit((p) => ({ ...p, weekday: Number(e.target.value || 0), error: "" }))}>
              ...
              <select style={editorInputSt} value={groupSlotEdit.roomName || primaryRoomName} onChange={(e) => setGroupSlotEdit((p) => ({ ...p, roomName: e.target.value }))}>
                {allKnownRooms.map((room) => <option key={room} value={room}>{room}</option>)}
              </select>
              <select style={editorInputSt} value={groupSlotEdit.trainerId || ""} onChange={(e) => setGroupSlotEdit((p) => ({ ...p, trainerId: e.target.value }))}>
                <option value="">Без тренера</option>
                {safeTrainers.map((t) => <option key={t.id} value={t.id}>{getTrainerDisplayName(t) || t.email || t.id}</option>)}
              </select>
              <input style={editorInputSt} placeholder="Нотатка" value={groupSlotEdit.note || ""} onChange={(e) => setGroupSlotEdit((p) => ({ ...p, note: e.target.value }))} />
              {groupSlotEdit.error ? <div style={{ color: theme.danger, fontSize: 12 }}>{groupSlotEdit.error}</div> : null}
              <div style={{ fontSize: 12, color: theme.textLight }}>
                Це змінить усі повторювані заняття цього слоту, не лише обрану дату. Single-date overrides не видаляються і лишаються пріоритетними для своїх дат.
              </div>
            </div>
            <div style={{ display: "flex", gap: 6, marginTop: 10, flexWrap: "wrap" }}>
