...
export default function ScheduleTab({
  ...
  const editorInputSt = {
    ...inputSt,
    background: isDarkTheme ? "rgba(15,23,42,.5)" : "#ffffff",
    color: theme.text,
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      ...
      {canManageBookings && showForm && (
        <div style={{ position: "fixed", inset: 0, zIndex: 5100, background: isDarkTheme ? "rgba(6,10,22,.62)" : "rgba(15,23,42,.24)", display: "grid", placeItems: isMobile ? "end center" : "center", padding: isMobile ? "0 8px" : 12, overflowX: "hidden" }}>
        <div style={{ ...cardSt, border: `1px solid ${theme.border}`, width: isMobile ? "calc(100vw - 16px)" : (formMode === "compact" ? "min(560px, 96vw)" : "min(620px, 96vw)"), maxWidth: isMobile ? "none" : undefined, maxHeight: isMobile ? "84vh" : "92vh", overflowY: "auto", overflowX: "hidden", borderRadius: isMobile ? "16px 16px 0 0" : 20, background: isDarkTheme ? "linear-gradient(180deg, rgba(22,30,46,.96), rgba(12,18,32,.96))" : "linear-gradient(180deg, #ffffff, #f8fafc)" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            ...
            <button style={{ ...btnS, minHeight: isMobile ? 36 : undefined, padding: isMobile ? "0 10px" : undefined, fontSize: isMobile ? 12 : undefined }} onClick={() => { setShowForm(false); setEditingId(null); setFormErrors({}); }}>✕</button>
          </div>
          <div
            style={{
              display: "grid",
          ...
          >
            ...
            <button type="button" style={{ ...btnS, minHeight: isMobile ? 40 : 34 }} onClick={applyFullToCompactForm}>Сховати зайві поля</button>
            </> : null}
          </div>
