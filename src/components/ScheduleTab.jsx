...
export default function ScheduleTab({
  ...
  const renderEventCardContent = (e, height, { reserveMenuSpace = false, statusText = "" } = {}) => {
    const isTinyCard = height < 34;
    const isShortCard = height < 56;
    const titleClamp = height > 54 ? 2 : 1;
    const primaryMeta = e.trainer && e.trainer !== "—" ? e.trainer : getEventTypeLabel(e.eventType);
    const extraMeta = [
      statusText,
      !isMobile ? getEventTypeLabel(e.eventType) : "",
      !isMobile && e.description ? e.description : "",
      !isMobile && e.peopleCount ? `${e.peopleCount} ос.` : "",
    ].filter(Boolean).join(" · ");
    return (
      <div style={{ display: "grid", gridTemplateRows: "minmax(0,auto) auto", gap: isTinyCard ? 0 : 1, height: "100%", minWidth: 0, overflow: "hidden" }}>
        <div style={{ color: theme.text, fontSize: isMobile ? 10.5 : 11.5, fontWeight: 850, lineHeight: "1.08em", paddingRight: reserveMenuSpace ? (isMobile ? 18 : 26) : 0, display: "-webkit-box", WebkitLineClamp: titleClamp, WebkitBoxOrient: "vertical", overflow: "hidden", minWidth: 0, wordBreak: "break-word" }}>
          {e.title}
        </div>
        <div style={{ display: "flex", alignItems: "baseline", gap: 4, minWidth: 0, overflow: "hidden", color: theme.textLight, lineHeight: "1.05em" }}>
          <span style={{ flex: "0 0 auto", color: theme.text, fontSize: isMobile ? 9.5 : 10.5, fontWeight: 800, whiteSpace: "nowrap" }}>{e.startTime}–{e.endTime}</span>
          {!isTinyCard ? <span style={{ flex: "1 1 auto", minWidth: 0, fontSize: isMobile ? 9 : 10, whiteSpace: "nowrap", textOverflow: "ellipsis", overflow: "hidden" }}>{primaryMeta}</span> : null}
        </div>
        {!isShortCard && extraMeta ? (
          <div style={{ minWidth: 0, color: theme.textLight, fontSize: isMobile ? 8.5 : 9.5, lineHeight: "1.05em", whiteSpace: "nowrap", textOverflow: "ellipsis", overflow: "hidden" }}>
            {extraMeta}
          </div>
        ) : null}
      </div>
                ...
                >
                  ...
                  {dayEvents.map((e) => {
                    ...
                    return (
                      ...
                      >
                        {renderEventCardContent(e, height, { reserveMenuSpace: isAdmin || e.kind === "booking" || canEditGroupSingleLesson(e), statusText: e.kind === "booking" ? stView.text : "" })}
