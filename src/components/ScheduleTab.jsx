...
const getEventTypeSticker = (event) => {
  const stickers = {
    group_lesson: "Гр",
    individual_training: "Інд",
    room_booking: "Рез",
    cleaning: "Кл",
    custom_admin_event: "Под",
  };
  const text = stickers[event?.eventType || event?.type] || "";
  return text ? { text, tone: "type" } : null;
};
const getEventCardStickers = (event, { includeExtra = false } = {}) => {
  const stickers = [];
  const initials = getTrainerInitials(event?.trainer);
  if (initials) stickers.push({ text: initials, tone: "trainer" });
  const typeSticker = getEventTypeSticker(event);
  if (typeSticker) stickers.push(typeSticker);
  if (includeExtra && event?.kind === "booking") {
    if (event.status === "cancelled") stickers.push({ text: "Скас", tone: "danger" });
    else if (event.status === "tentative") stickers.push({ text: "Попер", tone: "warn" });
    else if (event.peopleCount && event.eventType === "room_booking") stickers.push({ text: `${event.peopleCount}ос`, tone: "count" });
  }
  return stickers;
};
                ...
                <div key={room} style={{ border: `1px solid ${theme.border}`, borderRadius: 14, overflow: "hidden", minWidth: isMobile ? (selectedRoom === "all" && dayRoomColumnLimit !== "1" ? 240 : "100%") : 240, width: isMobile && selectedRoom === "all" && dayRoomColumnLimit !== "1" ? 240 : undefined }}>
                  ...
                  <div style={{ position: "relative", minHeight: (DAY_END_HOUR - DAY_START_HOUR) * 42, background: "rgba(255,255,255,.01)" }}>
                    ...
                    {items.sort((a,b)=>a.startMin-b.startMin).map((e) => {
                      ...
                      const maxStickers = height >= 96 ? 3 : height >= 54 ? 2 : 0;
                      const cardStickers = maxStickers ? getEventCardStickers(e, { includeExtra: height >= 96 }).slice(0, maxStickers) : [];
                      const stickerSt = (sticker) => {
                        const tone = sticker.tone || "type";
                        const toneStyle = tone === "trainer"
                          ? { background: isDarkTheme ? "rgba(15,23,42,.82)" : "rgba(30,41,59,.9)", borderColor: isDarkTheme ? "rgba(148,163,184,.32)" : "rgba(15,23,42,.14)", color: "#fff" }
                          : tone === "danger"
                            ? { background: isDarkTheme ? "rgba(220,38,38,.38)" : "rgba(254,226,226,.92)", borderColor: "rgba(220,38,38,.35)", color: isDarkTheme ? "#fee2e2" : "#991b1b" }
                            : tone === "warn"
                              ? { background: isDarkTheme ? "rgba(202,138,4,.34)" : "rgba(254,243,199,.94)", borderColor: "rgba(202,138,4,.35)", color: isDarkTheme ? "#fef3c7" : "#854d0e" }
                              : tone === "count"
                                ? { background: isDarkTheme ? "rgba(20,184,166,.28)" : "rgba(204,251,241,.88)", borderColor: "rgba(20,184,166,.34)", color: isDarkTheme ? "#ccfbf1" : "#115e59" }
                                : { background: isDarkTheme ? `${c.border}44` : `${c.border}24`, borderColor: `${c.border}88`, color: theme.text };
                        return { display: "inline-flex", alignItems: "center", justifyContent: "center", height: isMobile ? 14 : 16, padding: isMobile ? "2px 5px" : "2px 6px", borderRadius: 999, border: `1px solid ${toneStyle.borderColor}`, background: toneStyle.background, color: toneStyle.color, boxShadow: isDarkTheme ? "0 1px 2px rgba(0,0,0,.18)" : "0 1px 2px rgba(15,23,42,.08)", fontSize: isMobile ? 9.5 : 10.5, fontWeight: 700, lineHeight: 1, whiteSpace: "nowrap", boxSizing: "border-box" };
                      };
                      return (
                        <div
                          key={e.id}
                        ...
                        >
                          ...
                          {cardStickers.length ? (
                            <div style={{ display: "flex", gap: 3, alignItems: "center", flexWrap: "nowrap", overflow: "hidden", marginTop: 2 }}>
                              {cardStickers.map((sticker, index) => <span key={`${sticker.text}-${index}`} style={stickerSt(sticker)}>{sticker.text}</span>)}
                            </div>
                          ) : null}
                        </div>
                      );
