...
export default function ScheduleTab({
  ...
  const renderEventCardContent = (e, height, { reserveMenuSpace = false } = {}) => {
    const isShortCard = height < 42;
    const isCompactCard = height < 58;
    const metaParts = [
      e.trainer && e.trainer !== "—" ? e.trainer : "",
      e.kind === "booking" ? (statusStyles[e.status] || statusStyles.active).text : getEventTypeLabel(e.eventType),
      !isMobile && e.kind === "booking" ? getEventTypeLabel(e.eventType) : "",
    ].filter(Boolean);
    const metaText = metaParts.join(" · ") || getEventTypeLabel(e.eventType);
    return (
      <div
        style={{
          display: "flex",
          ...
          minWidth: 0,
          overflow: "hidden",
          paddingRight: reserveMenuSpace ? (isMobile ? 18 : 26) : 0,
        }}
      >
        <div
          style={{
            flex: "0 0 clamp(42px, 38%, 64px)",
            maxWidth: "44%",
            minWidth: 0,
            overflow: "hidden",
        ...
        >
          <div style={{ color: theme.text, fontSize: isMobile ? 9.5 : 10.5, fontWeight: 800, whiteSpace: "nowrap", textOverflow: "ellipsis", overflow: "hidden" }}>
            {e.startTime}–{e.endTime}
          </div>
          {!isShortCard ? (
            <div style={{ marginTop: 1, fontSize: isMobile ? 8.5 : 9.5, whiteSpace: "nowrap", textOverflow: "ellipsis", overflow: "hidden" }}>
              {metaText}
            </div>
          ) : null}
        </div>
        <div
          style={{
            ...
            lineHeight: "1.08em",
            display: "-webkit-box",
            WebkitLineClamp: isCompactCard ? 1 : 2,
                ...
                >
                  ...
                  {dayEvents.map((e) => {
                    ...
                    return (
                      ...
                      >
                        {renderEventCardContent(e, height, { reserveMenuSpace: isAdmin || e.kind === "booking" || canEditGroupSingleLesson(e) })}
