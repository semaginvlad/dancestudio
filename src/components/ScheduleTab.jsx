...
const statusStyles = { active: { opacity: 1, text: "active" }, tentative: { opacity: 0.65, text: "tentative" }, cancelled: { opacity: 0.45, text: "cancelled" } };
const palette = {
...
export default function ScheduleTab({
  ...
  const eventsByDay = useMemo(() => {
    ...
    safeBookings.forEach((b) => {
      ...
      const recurrence = recurrenceModes.includes(b.recurrence)
        ? b.recurrence
        : "none";
      ...
      weekDays.forEach((day) => {
        ...
        if (recurrence === "none" && dateKey !== b.date) return;
        if (recurrence !== "none") {
          if (day < baseDate) return;
          if (recurrence === "weekly" && day.getDay() !== baseDate.getDay())
            return;
          if (recurrence === "monthly" && day.getDate() !== baseDate.getDate())
            return;
        }
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
      ...
      {selectedEventDetails && (
        <div style={{ position: "fixed", inset: 0, zIndex: 3200, background: "rgba(0,0,0,.45)", display: "grid", placeItems: "center" }}>
          <div style={{ ...cardSt, border: `1px solid ${theme.border}`, width: "min(560px,92vw)" }}>
            <b>Деталі заняття</b>
      ...
      {isAdmin && groupSlotEdit && (
        <div style={{ position: "fixed", inset: 0, zIndex: 3200, background: "rgba(0,0,0,.45)", display: "grid", placeItems: "center" }}>
          <div style={{ ...cardSt, border: `1px solid ${theme.border}`, width: "min(620px,94vw)" }}>
                  ...
                  {dayEvents.map((e) => {
                    ...
                    return (
                      ...
                      >
                                ...
                                createPortal(
                                  ...
                                  {e.kind === "booking" ? (
                                    <>
                                      ...
                                      <button style={btnS} onClick={async () => { setOpenMenuState(null); await onUpdateBooking(e.id, { status: "tentative" }); }}>tentative</button>
                                      <button style={btnS} onClick={async () => { setOpenMenuState(null); await onUpdateBooking(e.id, { status: "cancelled" }); }}>cancelled</button>
                                      <button style={btnS} onClick={async () => { setOpenMenuState(null); await onUpdateBooking(e.id, { status: "active" }); }}>active</button>
                                      <button style={btnS} onClick={async () => { setOpenMenuState(null); await onUpdateBooking(e.id, { color: null }); }}>reset color</button>
                                    </>
                                  ) : (
                                    <>
                                      <button
                        ...
                        <div>{e.trainer}</div>
                        {e.kind === "booking" ? <div>{stView.text}</div> : null}
