...
export default function ScheduleTab({
  ...
  const [groupOverrideEdit, setGroupOverrideEdit] = useState(null); // single generated group lesson override draft
  ...
  const groupOverrideMap = useMemo(() => {
    const map = new Map();
    safeGroupLessonOverrides.forEach((override) => {
      if (!override?.groupId || !override?.date || override?.slotIndex == null) return;
  ...
  const buildEventsByDayForDates = useMemo(() => (dates = []) => {
    ...
    safeGroups.forEach((g) =>
      (Array.isArray(g.schedule) ? g.schedule : []).forEach((row, idx) => {
        ...
        slots.push({
          ...
          title: g.name || g.id,
          directionId: g.directionId || g.direction_id || null,
          direction: getDirectionDisplayName(dirMap.get(String(g.directionId || g.direction_id || "")) || "—"),
          trainerId: g.trainer_id || g.trainerId || null,
          trainer: trainerMap.get(String(g.trainer_id || g.trainerId || "")) || "—",
        });
      }),
    );
    normalizedDates.forEach((d) => {
      ...
      slots
        ...
        .forEach((s) => {
          const override = groupOverrideMap.get(`${s.groupId}:${date}:${s.slotIndex}`);
          ...
          if (st == null) return;
          const en = toMin(endTime) ?? st + 60;
          const trainerId = override?.trainerId || s.trainerId || null;
          const title = override?.title?.trim() || s.title;
          const roomName = normalizeRoomName(override?.roomName) || getRoomLabel(s) || primaryRoomName;
          map.get(date).push({
            ...s,
            id: override?.id ? `${s.id}:override:${override.id}` : s.id,
            kind: "group",
            ...
            roomName,
            trainerId,
            trainer: trainerMap.get(String(trainerId || "")) || s.trainer || "—",
            note: override?.note || "",
            isOverride: !!override,
  ...
  const canMutateGroupEvent = (event) =>
    isAdmin || (event?.kind === "group" && String(event.trainerId || "") === currentTrainerId);
  ...
  const openGroupOverrideEditor = (event) => {
    if (!canMutateGroupEvent(event)) {
    ...
    setGroupOverrideEdit({
      ...
      date: event.date,
      groupName: event.originalTitle || event.groupName || event.title || "Групове заняття",
      ...
      roomName: normalizeRoomName(event.roomName || primaryRoomName) || primaryRoomName,
      trainerId: isAdmin ? event.trainerId || "" : currentTrainerId,
  ...
  const updateGroupOverrideDraft = (patch) =>
    setGroupOverrideEdit((prev) => ({ ...(prev || {}), ...patch, error: "" }));
  ...
  const saveGroupOverrideEdit = async () => {
    ...
    if (!groupOverrideEdit.groupId || !groupOverrideEdit.date || groupOverrideEdit.slotIndex == null) {
      updateGroupOverrideDraft({ error: "Не вдалося визначити заняття для override" });
      return;
    }
    if (st == null || en == null || en <= st) {
      updateGroupOverrideDraft({ error: "Перевірте час початку та завершення" });
      return;
    }
    const payload = {
      ...
      originalStartTime: groupOverrideEdit.originalStartTime || groupOverrideEdit.startTime,
      originalEndTime: groupOverrideEdit.originalEndTime || groupOverrideEdit.endTime,
      ...
      title: String(groupOverrideEdit.title || "").trim() || null,
      note: String(groupOverrideEdit.note || "").trim() || null,
      status: "active",
    };
    try {
      if (groupOverrideEdit.overrideId) {
      ...
      console.error(err);
      updateGroupOverrideDraft({ error: "Не вдалося зберегти зміну тільки цього заняття" });
    }
  };

  const resetGroupOverride = async () => {
    if (!groupOverrideEdit?.overrideId || !onDeleteGroupLessonOverride) return;
    try {
      await onDeleteGroupLessonOverride(groupOverrideEdit.overrideId);
      setGroupOverrideEdit(null);
    } catch (err) {
      console.error(err);
      updateGroupOverrideDraft({ error: "Не вдалося прибрати override" });
    }
  };
  ...
  return (
    <div style={{ display: "grid", gap: 12 }}>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 5000, background: editorOverlay, display: "grid", placeItems: "center", padding: 12 }}>
          <div style={{ ...cardSt, border: `1px solid ${theme.border}`, width: "min(480px,94vw)", maxHeight: "86vh", overflow: "auto", borderRadius: 18, background: editorSurface, color: theme.text }}>
            ...
            <div style={{ marginTop: 12, display: "grid", gap: 8, fontSize: 13 }}>
              ...
              {selectedEventDetails.isOverride ? <div style={{ color: theme.textLight }}>Змінено тільки це заняття</div> : null}
            </div>
            {selectedEventDetails.kind === "group" && canMutateGroupEvent(selectedEventDetails) ? (
      ...
      {groupOverrideEdit && (
        <div style={{ position: "fixed", inset: 0, zIndex: 5000, background: editorOverlay, display: "grid", placeItems: "center", padding: 12 }}>
          <div style={{ ...cardSt, border: `1px solid ${theme.border}`, width: "min(520px,94vw)", maxHeight: "90vh", overflow: "auto", borderRadius: 18, background: editorSurface, color: theme.text }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start", gap: 10 }}>
              <div>
                <div style={editorSectionLabelSt}>Один день</div>
                <b style={{ fontSize: 18 }}>Змінити тільки це заняття</b>
                <div style={{ color: theme.textLight, fontSize: 12, marginTop: 4 }}>{groupOverrideEdit.groupName} · {groupOverrideEdit.date}</div>
              </div>
              <button style={{ ...editorBtnSt, minHeight: 32, width: 34, padding: 0 }} onClick={() => setGroupOverrideEdit(null)}>✕</button>
            </div>
            <div style={{ marginTop: 10, padding: 10, borderRadius: 14, border: `1px solid ${theme.border}`, color: theme.textLight, fontSize: 12, background: isDarkTheme ? "rgba(15,23,42,.32)" : "rgba(255,255,255,.56)" }}>
              Це створить override для конкретної дати. Регулярний графік групи не змінюється.
              {/* TODO Phase 2: integrate group lesson overrides with Attendance, salary, Telegram reminders and analytics. */}
            </div>
            {groupOverrideEdit.error ? <div style={{ marginTop: 10, color: theme.danger, fontSize: 13 }}>{groupOverrideEdit.error}</div> : null}
            <div style={{ display: "grid", gap: 10, marginTop: 12 }}>
              <label style={{ display: "grid", gap: 5, fontSize: 12, color: theme.textLight }}>
                Назва / нотатка на картці
                <input style={editorInputSt} value={groupOverrideEdit.title || ""} onChange={(e) => updateGroupOverrideDraft({ title: e.target.value })} placeholder={groupOverrideEdit.groupName || "Групове заняття"} />
              </label>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8 }}>
                <label style={{ display: "grid", gap: 5, fontSize: 12, color: theme.textLight }}>
                  Дата
                  <input style={{ ...editorInputSt, opacity: .78 }} type="date" value={groupOverrideEdit.date || ""} readOnly />
                </label>
                <label style={{ display: "grid", gap: 5, fontSize: 12, color: theme.textLight }}>
                  Початок
                  <input style={editorInputSt} type="time" value={groupOverrideEdit.startTime || ""} onChange={(e) => updateGroupOverrideDraft({ startTime: e.target.value })} />
                </label>
                <label style={{ display: "grid", gap: 5, fontSize: 12, color: theme.textLight }}>
                  Кінець
                  <input style={editorInputSt} type="time" value={groupOverrideEdit.endTime || ""} onChange={(e) => updateGroupOverrideDraft({ endTime: e.target.value })} />
                </label>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                <label style={{ display: "grid", gap: 5, fontSize: 12, color: theme.textLight }}>
                  Зала
                  <select style={editorInputSt} value={groupOverrideEdit.roomName || primaryRoomName} onChange={(e) => updateGroupOverrideDraft({ roomName: e.target.value })}>
                    {allKnownRooms.map((room) => <option key={room} value={room}>{room}</option>)}
                  </select>
                </label>
                <label style={{ display: "grid", gap: 5, fontSize: 12, color: theme.textLight }}>
                  Тренер
                  <select style={editorInputSt} value={groupOverrideEdit.trainerId || ""} onChange={(e) => updateGroupOverrideDraft({ trainerId: e.target.value })} disabled={!isAdmin}>
                    <option value="">Без тренера</option>
                    {safeTrainers.map((trainer) => (
                      <option key={trainer.id} value={trainer.id}>{getTrainerDisplayName(trainer)}</option>
                    ))}
                  </select>
                </label>
              </div>
              <label style={{ display: "grid", gap: 5, fontSize: 12, color: theme.textLight }}>
                Нотатка
                <textarea style={{ ...editorInputSt, minHeight: 78, paddingTop: 10, resize: "vertical" }} value={groupOverrideEdit.note || ""} onChange={(e) => updateGroupOverrideDraft({ note: e.target.value })} placeholder="Коментар тільки для цього заняття" />
              </label>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 8, flexWrap: "wrap", marginTop: 14, paddingTop: 12, borderTop: `1px solid ${theme.border}` }}>
              <div>
                {groupOverrideEdit.overrideId && onDeleteGroupLessonOverride ? (
                  <button style={{ ...editorBtnSt, color: theme.danger }} onClick={resetGroupOverride}>Скинути override</button>
                ) : null}
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <button style={editorBtnSt} onClick={() => setGroupOverrideEdit(null)}>Скасувати</button>
      ...
      {isAdmin && groupSlotEdit && (
        <div style={{ position: "fixed", inset: 0, zIndex: 5000, background: "rgba(0,0,0,.45)", display: "grid", placeItems: "center" }}>
          ...
          <div style={{ marginTop: 8, padding: 10, borderRadius: 12, border: `1px solid ${theme.border}`, color: theme.textLight, fontSize: 12 }}>
