...
export function SubForm({ initial, onDone, onCancel, students, groups, studentGrps, subs = [] }) {
  ...
  const [manualEndDate, setManualEndDate] = useState(initial?.endDate || addMonth(initial?.startDate || today()));
  ...
  const selectedEndDate = retrospectiveMode ? manualEndDate : endDate;
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        {retrospectiveMode ? (
          <Field label="Кінець (вручну)">
            <input style={{ ...inputSt, cursor: "pointer", height: "52px" }} type="date" value={manualEndDate} onChange={e => setManualEndDate(e.target.value)} onClick={e => e.target.showPicker && e.target.showPicker()} />
          </Field>
        ) : (
