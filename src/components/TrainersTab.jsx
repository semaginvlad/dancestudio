...
const DEBUG_TRAINERS_REVENUE = false;
...
const rangeOverlaps = (leftStart, leftEnd, rightStart, rightEnd) => {
  if (!leftStart || !rightStart) return false;
  const aEnd = leftEnd || "9999-12-31";
  const bEnd = rightEnd || "9999-12-31";
...
const getNominalAmount = (sub = {}) =>
  money(sub.nominalAmount || sub.nominal_amount || sub.fullAmount || sub.full_amount || sub.price || sub.basePrice || sub.base_price || sub.baseAmount || sub.base_amount);
const getDiscountSide = (sub = {}) => {
  const raw = String(sub.discountSide || sub.discount_side || sub.discountBy || sub.discount_by || sub.discountSource || sub.discount_source || "").toLowerCase().trim();
  if (["studio", "студія", "studioside", "studio_side", "by_studio"].includes(raw)) return "studio";
  if (["trainer", "тренер", "trainerside", "trainer_side", "by_trainer"].includes(raw)) return "trainer";
  return raw;
};
const getDiscountPercent = (sub = {}) => {
  const rawValue = sub.discountPercent ?? sub.discount_percent ?? sub.discountPct ?? sub.discount_pct ?? 0;
  const raw = Number(String(rawValue).replace(",", "."));
...
const getRawDiscountSnapshot = (sub = {}) => ({
  ...
  discount_source: sub.discount_source,
...
const getEffectivePaidAmount = (sub = {}) => {
  ...
  const discountPct = getDiscountPercent(sub);
  if (amount > 0 && nominal > 0 && amount <= nominal) return amount;
  if (amount > 0 && nominal <= 0) return amount;
  const discountValue = getDiscountValue(sub);
  if (side === "studio" && nominal > 0 && amount > 0 && amount < nominal) return amount;
  if (side === "studio" && nominal > 0 && discountPct > 0) return Math.max(0, nominal - Math.round((nominal * discountPct) / 100));
  if (side === "studio" && nominal > 0 && discountValue > 0) return Math.max(0, nominal - discountValue);
  if (nominal > 0 && discountValue > 0) return Math.max(0, nominal - discountValue);
...
const calcTrainerShare = ({ source = "subscription", row = {}, trainerPct = 0 }) => {
  ...
  const side = getDiscountSide(row);
...
export default function TrainersTab({
  ...
  const buildGroupCardsForRange = (start, end) => trainerBoundGroups.map((g) => {
    ...
    const subsById = new Map(subs.map((s) => [String(s.id), s]));
    ...
    const attendanceStudentIds = new Set(
      groupAttnRows
        .map((a) => a.studentId || subsById.get(String(a.subId || ""))?.studentId || null)
        .filter(Boolean)
        .map(String)
    );
    const periodGroupSubs = subs.filter((s) => {
      if (String(s.groupId) !== String(g.id)) return false;
      const subStart = String(s.activationDate || s.startDate || s.created_at || "").slice(0, 10);
      const subEnd = String(s.endDate || "").slice(0, 10) || null;
      return rangeOverlaps(subStart, subEnd, start, effectiveEnd || end);
    });
    const subscriptionStudentIds = new Set(periodGroupSubs.map((s) => String(s.studentId)).filter(Boolean));
    const groupStudentIds = new Set([...attendanceStudentIds, ...subscriptionStudentIds]);
    const groupStudents = groupStudentIds.size;
    const groupActiveSubs = periodGroupSubs.filter((s) => s.status !== "expired").length;
  ...
  const groupCards = useMemo(
    ...
    [attn, cancelledSet, range.end, range.start, subs, trainerBoundGroups],
  );
  const groupCardsPrev = useMemo(
    () => buildGroupCardsForRange(rangePrev.start, rangePrev.end),
    [attn, cancelledSet, rangePrev.end, rangePrev.start, subs, trainerBoundGroups],
  );
  ...
  useEffect(() => {
    if (!selectedTrainerId || !DEBUG_TRAINERS_REVENUE) return;
  ...
  const revenueAnalytics = useMemo(() => {
    ...
    const groupNameById = Object.fromEntries(trainerBoundGroups.map((g) => [normalizeRevenueGroupId(g.id), g.name || "—"]));
    const trainerNameByGroup = Object.fromEntries(trainerBoundGroups.map((g) => [normalizeRevenueGroupId(g.id), g.trainerName || g.coachName || "—"]));
    const studentNameById = Object.fromEntries(scopedData.scopedStudents.map((s) => [String(s.id), [s.lastName, s.firstName].filter(Boolean).join(" ").trim() || s.name || "—"]));
    const perGroupCurrent = {};
    const perGroupPrev = {};
    const perTrainerCurrent = {};
    const perTrainerPrev = {};
    const debugRows = [];
    const guestOneOffStats = {
      guestOneOffCount: 0,
      guestOneOffQuantityTotal: 0,
      guestOneOffRevenueTotal: 0,
      possibleDuplicates: [],
    };
    const seenGuestIdsCurrent = new Set();
    const seenGuestSignatureCurrent = new Map();
    let monthCurrent = 0;
    let monthPrev = 0;
    ...
    paidSubsCurrent.forEach((s) => {
      ...
      const debug = calcTrainerShare({ source: "subscription", row: { ...s, studentName: studentNameById[String(s.studentId)] }, trainerPct: pctValue });
      const trainerShare = debug.trainerShare;
      monthCurrent += trainerShare;
      perGroupCurrent[groupId] = (perGroupCurrent[groupId] || 0) + trainerShare;
      perTrainerCurrent[trainerNameByGroup[groupId]] = (perTrainerCurrent[trainerNameByGroup[groupId]] || 0) + trainerShare;
      debugRows.push({ ...debug, groupName: groupNameById[groupId], trainerName: trainerNameByGroup[groupId], includedInCurrentPeriod: true });
    });
    paidSubsPrev.forEach((s) => {
      const groupId = normalizeRevenueGroupId(s.groupId);
      const pctValue = trainerPctByGroup[groupId] ?? 0;
      const trainerShare = calcTrainerShare({ source: "subscription", row: s, trainerPct: pctValue }).trainerShare;
      monthPrev += trainerShare;
      perGroupPrev[groupId] = (perGroupPrev[groupId] || 0) + trainerShare;
      perTrainerPrev[trainerNameByGroup[groupId]] = (perTrainerPrev[trainerNameByGroup[groupId]] || 0) + trainerShare;
    });
    const addGuestOneOff = (rows, collector, periodLabel) => {
      rows.forEach((a) => {
        if (a.studentId || a.student_id) return;
        const rawEntryType = String(a.entryType || "").trim().toLowerCase();
        if ((rawEntryType === "trial" || rawEntryType === "single") && !a.guestType) {
          if (collector === perGroupCurrent) {
            guestOneOffStats.possibleDuplicates.push({
              reason: "invalid attendance row ignored (guestType null)",
              period: periodLabel,
        ...
        const effective = getAttendanceEffectiveType(a, scopedData.scopedSubs);
        if (effective !== "trial" && effective !== "single") return;
        const groupId = normalizeRevenueGroupId(a.groupId);
        if (!(groupId in trainerPctByGroup)) return;
        const qty = Math.max(1, Number(a.quantity || 1));
        const dedupeIdentity = a.studentId || a.guestName || "anonymous";
        const dedupeKey = `${String(a.date || "").slice(0, 10)}|${groupId}|${dedupeIdentity}|${effective}`;
        if (collector === perGroupCurrent && a.id && seenGuestIdsCurrent.has(String(a.id))) {
          guestOneOffStats.possibleDuplicates.push({
            reason: "duplicate guest one-off excluded from salary",
            period: periodLabel,
            attendanceId: a.id,
            dedupeKey,
          });
        ...
        if (collector === perGroupCurrent && a.id) seenGuestIdsCurrent.add(String(a.id));
        if (collector === perGroupCurrent) {
          const signatureCount = seenGuestSignatureCurrent.get(dedupeKey) || 0;
          seenGuestSignatureCurrent.set(dedupeKey, signatureCount + 1);
          if (signatureCount > 0) {
            guestOneOffStats.possibleDuplicates.push({
              reason: "same signature appears multiple times (kept in salary)",
              period: periodLabel,
              attendanceId: a.id || null,
              dedupeKey,
            });
        ...
        const pctValue = trainerPctByGroup[groupId] ?? 0;
        const debug = calcTrainerShare({ source: "attendance_guest_oneoff", row: { ...a, planType: effective, studentName: studentNameById[String(a.studentId)] }, trainerPct: pctValue });
        const trainerShare = debug.trainerShare * qty;
        collector[groupId] = (collector[groupId] || 0) + trainerShare;
        if (collector === perGroupCurrent) {
          guestOneOffStats.guestOneOffCount += 1;
          guestOneOffStats.guestOneOffQuantityTotal += qty;
          guestOneOffStats.guestOneOffRevenueTotal += (debug.attendanceRevenue || 0) * qty;
          perTrainerCurrent[trainerNameByGroup[groupId]] = (perTrainerCurrent[trainerNameByGroup[groupId]] || 0) + trainerShare;
          debugRows.push({
            ...debug,
            quantity: qty,
            ...
            trainerName: trainerNameByGroup[groupId],
            includedInCurrentPeriod: true,
          });
        }
        if (collector === perGroupPrev) perTrainerPrev[trainerNameByGroup[groupId]] = (perTrainerPrev[trainerNameByGroup[groupId]] || 0) + trainerShare;
      });
    };
    const currRows = scopedData.scopedAttn.filter((a) => inRange(a.date, range.start, range.end));
    const prevRows = scopedData.scopedAttn.filter((a) => inRange(a.date, rangePrev.start, rangePrev.end));
    addGuestOneOff(currRows, perGroupCurrent, "current");
    addGuestOneOff(prevRows, perGroupPrev, "previous");
    monthCurrent = Object.values(perGroupCurrent).reduce((s, x) => s + x, 0);
    monthPrev = Object.values(perGroupPrev).reduce((s, x) => s + x, 0);
    ...
    return {
      ...
      debug: {
        ...
        guestOneOffSummary: guestOneOffStats,
      },
    };
  }, [paidSubsCurrent, paidSubsPrev, trainerBoundGroups, scopedData.scopedAttn, scopedData.scopedSubs, range.start, range.end, rangePrev.start, rangePrev.end]);
    ...
    for (let i = months - 1; i >= 0; i -= 1) {
      ...
      const attendanceTotal = scopedData.scopedAttn
        ...
        .reduce((sum, s) => {
          ...
          return sum + calcTrainerShare({ source: "subscription", row: s, trainerPct: pctValue }).trainerShare;
        }, 0);
  ...
  return (
    ...
    >
      ...
      <section style={{ display: "flex", flexDirection: "column", gap: 10, minWidth: 0, width: "100%", maxWidth: "100%", boxSizing: "border-box" }}>
        ...
        {DEBUG_TRAINERS_REVENUE && <details style={{ ...card(), padding: 12 }}>
          <summary style={{ cursor: "pointer", fontWeight: 800 }}>DEBUG: розрахунок ЗП тренерів</summary>
          <div style={{ marginTop: 8, fontSize: 12, color: theme.textSoft }}>Total computed salary: <b style={{ color: theme.text }}>{revenueAnalytics.debug.totalComputedSalary.toLocaleString()} ₴</b></div>
          ...
          <div style={{ marginTop: 8, fontSize: 12, color: theme.textSoft }}>Guest one-off summary: <pre style={{ whiteSpace: "pre-wrap", margin: 0 }}>{JSON.stringify(revenueAnalytics.debug.guestOneOffSummary, null, 2)}</pre></div>
