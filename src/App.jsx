...
// ═══════════════════════════════════════
// ВІДВІДУВАННЯ (З ЧЕРНЕТКОЮ ТА ЖУРНАЛОМ)
// ═══════════════════════════════════════
function AttendanceTab({ groups, subs, setSubs, attn, setAttn, studentMap, studentGrps, cancelled }) {
  const [viewMode, setViewMode] = useState("daily"); // "daily" або "journal"
  const [gid, setGid] = useState(groups[0]?.id || "");
  const [date, setDate] = useState(today());
  const [journalMonth, setJournalMonth] = useState(today().slice(0, 7)); // YYYY-MM
