 ...
 const subsExt = useMemo(()=>{
  ...
  return (
    <div className={tab === "messages" ? "app-messages-active" : undefined} data-theme-version={themeRenderTick} data-build-label={APP_BUILD_LABEL} style={{minHeight:"100dvh", background:theme.bg, color:theme.textMain, fontFamily:"'Poppins',sans-serif", paddingBottom: "max(100px, env(safe-area-inset-bottom))"}}>
      ...
      <Modal open={modal==="editWaitlist"} onClose={()=>{setModal(null);setEditItem(null)}} title="Редагувати запис резерву" variant={studentsMobileModalVariant}><WaitlistForm initial={editItem} submitLabel="Зберегти" onCancel={()=>{setModal(null);setEditItem(null)}} onDone={async(d)=>{try{const w=await db.updateWaitlist(editItem.id,d);setWaitlist(p=>p.map(x=>x.id===w.id?w:x));setModal(null);setEditItem(null);}catch(e){console.error("Failed to update waitlist entry:", e);alert(`Не вдалося оновити запис резерву: ${e?.message || e}`);}}} students={students} groups={activeGroups} studentGrps={studentGrps} directionsList={directionsList}/></Modal>
