...
export async function fetchCancelled() {
  ...
  return data.map(c => {
    let parsed = null;
    try {
      parsed = c.reason ? JSON.parse(c.reason) : null;
    } catch (_e) {}
...
export async function insertCancelled(c) {
  ...
  return {
    ...
    groupId: data.group_id,
    date: data.date,
    originalEnds: data.reason ? JSON.parse(data.reason) : null,
  };
}
...
export async function incrementUsed(subId, qty = 1) {
  ...
  if (sub) {
    await supabase.from('subscriptions').update({
      used_trainings: (sub.used_trainings || 0) + qty,
    }).eq('id', subId);
  }
}
...
export async function decrementUsed(subId, qty = 1) {
  ...
  if (sub) {
    await supabase.from('subscriptions').update({
      used_trainings: Math.max(0, (sub.used_trainings || 0) - qty),
    }).eq('id', subId);
  }
}
